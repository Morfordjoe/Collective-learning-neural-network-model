...
#Collective direction is the same as one of the individual NN output directions, randomly determined (weighted by leadership coefficient)
...
for j in range(pair_n):
    ...
    while (i <= Max):
        ...
        if leader_diff_raw > 0.5:
            ...
            leader_diff_raw = 1 + leader_diff_raw
        leader_diff = np.absolute(leader_diff_raw)
        ...
        if follower_diff_raw > 0.5:
            ...
            follower_diff_raw = 1 + follower_diff_raw
        follower_diff = np.absolute(follower_diff_raw)
        ...
        print("Pair difference:", pair_diff)

        l1 = np.random.choice([1, 0], size=1, p=[leader_1[0], leader_2[0]])
        #Determines whether NN1 or NN2 leads this iteration, probabilistically
        #Collective direction is the same direction as the (objective) direction output of the NN that leads

        if l1:
            collective_direction = abs_pred1
        else:
            collective_direction = abs_pred2
        ...
        leader_collective_membership_benefit = leader_diff - collective_error
        print("Leader collective membership benefit:", leader_collective_membership_benefit)

        follower_collective_membership_benefit = follower_diff - collective_error
        ...
        if (i<Max):
            ...
            #Then determines collective direction with collective decision making rule (one of thie individual outputs, probabilistically)
            #Then determines squared collective error -> output of loss function
            #If the other NN leads, then this NN cannot learn in this training trial as there is no relationship between output and loss function output
            def customLoss1(true, predict):
                predict_0_1 = (predict + diff_ego1) % 1
                joint_direction = predict_0_1*l1 + abs_pred2*l2
                error = K.abs(true - joint_direction)
            ...
            def customLoss2(true, predict):
                ...
                joint_direction = predict_0_1*l2 + abs_pred1*l1
                error = K.abs(true - joint_direction)
                corrected_error = K.switch(error>0.5, 1-error, error)
                return(K.square(corrected_error))
...
file_path = "/Users/joemorford/Desktop/CL_NNs/Sim_data/Despotic_pairs_N"+str(pair_n)+"Max"+str(Max)+"LR"+str(LR)+"MM"+str(MM)+"DEC"+str(DEC)+"L"+str(leadership_coeff) +".csv"
import csv
